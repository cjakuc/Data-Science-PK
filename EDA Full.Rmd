...
test.step %>%
  ...
  # view(test.step) %>%
    ggplot() + 
      geom_point(aes(x=Proportion_Attended_College,y=med.tract.household.inc),color="gray") + 
      theme_minimal() +
      geom_point(aes(x=Proportion_Attended_College,y=Pred,color=Model),size=1)+
      geom_hline(yintercept=0)+
      facet_grid(Model~.)+
      ggtitle("")+
      xlab("Proportion Attended College")+
      ylab("Median Tract Household Income")
...
test.step %>%
  ...
  # view(test.step) %>%
    ggplot() + 
      geom_point(aes(x=Poverty_Rate,y=med.tract.household.inc),color="gray") + 
      theme_minimal() +
      geom_point(aes(x=Poverty_Rate,y=Pred,color=Model),size=1)+
      geom_hline(yintercept=0)+
      facet_grid(Model~.)+
      ggtitle("")+
      xlab("Poverty Rate")+
...
```{r}
test.step1<-test.step %>%
  group_by(med.tract.household.inc) %>%
  dplyr::summarize(count=n()) %>%
  dplyr::ungroup() %>%
  right_join(test.step,by="med.tract.household.inc") %>%
  gather_predictions(model1,model2,model3,model4,model5,model6) %>%
  ggplot() +
    geom_point(mapping=aes(x=med.tract.household.inc,y=count))+
    geom_line(aes(x=med.tract.household.inc,y=pred),size=2)+
    theme_minimal()+
    facet_grid(model~.)
```
