...
```{r}
unique(PKcleaning1$Cause_Of_Death)

ggplot(data=PKcleaning1) +
  geom_bar(aes(x=Cause_Of_Death))

NoGun= filter(PKcleaning1, Cause_Of_Death != "Gunshot")
...
From the dataset it can be observed that most of the police killings occured in areas that have an unemployment rate that is above the national average of 3.6%. 438 of the 465 killings occured in areas that had unemployment rates above the national average meaning that higher unemployment rates could have a significant impact on where police killings occur.
```{r}
PK1=PoliceKillings%>%
  mutate(higherthanavg=if_else(Unemployment_Rate>0.036,"Yes","No"))
